...
def describe_instantaneous_preview_basis() -> str:
    return (
        ...
        "Domoticz IDX 20 Usage is the live grid-import watt reading we will encode back into that ABB format after clamping negatives to zero. "
        "House load is ignored."
    )
...
def decode_signed_scaled_watts(register_values: Sequence[int], *, offset: int = INSTANTANEOUS_ACTIVE_POWER_TOTAL_OFFSET) -> float | None:
    if len(register_values) < offset + INSTANTANEOUS_ACTIVE_POWER_TOTAL_REGISTER_LENGTH:
        return None
    ...
    if raw & 0x80000000:
        raw -= 0x100000000
    return raw * INSTANTANEOUS_ACTIVE_POWER_TOTAL_SCALE
